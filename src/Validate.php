...
class Validate
{
            ...
            if (false === $result) {
                ...
                if (!empty($msg[$i])) {
                    ...
                    if (is_string($message) && strpos($message, '{%') === 0) {
                        $message = Lang::getInstance()->get(substr($message, 2, -1), [], $this->getLangRange());
                    }
                } else {
     ...
     * @return bool
     */
    public function expire($value, $rule, int $requestTime)
    {
        if (is_string($rule)) {
            $rule = explode(',', $rule);
        }
    ...
    protected function getRuleMsg($attribute, $title, $type, $rule)
    {
        ...
        if (isset($this->message[$attribute . '.' . $type])) {
            ...
            $msg = $title . $lang->get('not conform to the rules', [], $langRange);
        }
        ...
        if (0 === strpos($msg, '{%')) {
            $msg = $lang->get(substr($msg, 2, -1), [], $langRange);
        } elseif ($lang->has($msg, $langRange)) {
            $msg = $lang->get($msg, [], $langRange);
        }
     ...
     */
    public function setLangRange(string $langRange = 'zh-cn')
    {
